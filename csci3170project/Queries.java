  ...
  static void initQueries(Connection conn) throws SQLException {
    ...
    selectBookByISBN = conn.prepareStatement("""
        SELECT title, book.isbn, unit_price, no_of_copies, author_name
          ...
          WHERE book.isbn = ? and book.isbn = book_author.isbn
          ORDER BY title, book.isbn, author_name
        """);
    selectBookByTitle = conn.prepareStatement("""
        SELECT title, book.isbn, unit_price, no_of_copies, author_name
          FROM book, book_author
          WHERE title = ? and book.isbn = book_author.isbn
          ORDER BY title, book.isbn, author_name
        """);
    ...
    selectCustomerByID = conn.prepareStatement("""
        SELECT customer_id
          FROM customer
          WHERE customer_id = ?
        """);
    ...
    checkCopiesAvailable = conn.prepareStatement("""
        SELECT no_of_copies - sum(quantity)
          FROM book, ordering
          WHERE book.isbn = ? and book.isbn = ordering.isbn
          group by book.isbn, no_of_copies
        """);

    getMaxOrderID = conn.prepareStatement("""
        SELECT MAX(order_id)
          FROM orders
        """);

    insertOrders = conn.prepareStatement("""
        INSERT INTO orders (order_id, o_date, shipping_status, customer_id)
          VALUES (?, ?, 'N', ?)
        """);
        ...
        UPDATE orders
          SET charge = (
              SELECT sum(unit_price * quantity)
                FROM book, ordering
                where order_id = ? and book.isbn = ordering.isbn
                group by order_id)
          WHERE order_id = ?
        """);
